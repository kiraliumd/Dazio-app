...
export function useEquipmentAvailability() {
  ...
  const checkAvailability = (
    ...
    const totalStock =
      equipmentStock[equipmentName as keyof typeof equipmentStock] || 0;
    ...
    const occupiedQuantity = conflictingRentals.reduce(
    ...
    const result: AvailabilityInfo = {
      ...
      conflictingRentals: conflictingRentals.map(rental => ({
        clientName:
          clientNames[rental.startDate as keyof typeof clientNames] ||
