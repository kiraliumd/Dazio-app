...
const RentalForm = lazy(() =>
  import('../../../components/rental-form').then(module => ({
    default: module.RentalForm,
  }))
);
...
export default function RentalsPage() {
  ...
  const handleSaveRental = async (
    rentalData: Omit<Rental, 'id'> & { id?: string }
  ) => {
    try {
      setSaving(true);
      ...
      // Tipo específico para dados do banco
      type RentalForDB = typeof rentalForDBBase & {
      ...
      const rentalForDB: RentalForDB = {
        ...rentalForDBBase,
        recurrence_type: rentalData.isRecurring
          ? rentalData.recurrenceType
          : undefined,
        recurrence_interval: rentalData.isRecurring
          ? rentalData.recurrenceInterval || 1
          : undefined,
        recurrence_end_date: rentalData.isRecurring
          ? rentalData.recurrenceEndDate || null
          : undefined,
        recurrence_status: rentalData.isRecurring
          ? (rentalData.recurrenceStatus as
              | 'active'
              | 'paused'
              | 'cancelled'
              | 'completed') || 'active'
          : undefined,
        parent_rental_id: rentalData.isRecurring
          ? rentalData.parentRentalId || null
          : undefined,
        next_occurrence_date: rentalData.isRecurring
          ? rentalData.nextOccurrenceDate || null
          : undefined,
      };

      // Para criação, garantir que os campos obrigatórios estejam presentes
      const rentalForCreate = {
        ...rentalForDBBase,
        recurrence_type: rentalData.isRecurring
          ? rentalData.recurrenceType || 'weekly'
          : 'weekly',
        recurrence_interval: rentalData.isRecurring
          ? rentalData.recurrenceInterval || 1
          : 1,
      ...
      if (rentalData.id) {
        ...
        // Criar nova locação
        await createRental(rentalForCreate, itemsForDB, {
          installation: new Date(
            rentalData.startDate + ' ' + rentalData.installationTime
          ),
      ...
      const contractData = {
        ...
        contract: {
          ...
          installationLocation:
            rental.installationLocation || 'Local não informado',
          totalValue: rental.totalValue || 0,
          discount: rental.discount || 0,
          finalValue: rental.finalValue || 0,
          observations: rental.observations || '', // ✅ Campo observations adicionado
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <CardDescription className="hidden sm:block">
                    Todos os contratos de locação cadastrados no sistema
                  </CardDescription>
                </div>
                <Button
                  onClick={() => {
                        ...
                        paginatedRentals.map(rental => (
                          <TableRow key={rental.id}>
                            ...
                            <TableCell>
                              <div className="font-medium text-foreground">
                                R${' '}
                                {rental.finalValue.toFixed(2).replace('.', ',')}
                              </div>
                            </TableCell>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-primary">
                        R${' '}
                        {viewingRental.finalValue.toFixed(2).replace('.', ',')}
                      </div>
                    ...
                    <CardContent className="space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-medium">
                          {formatDate(viewingRental.startDate)} -{' '}
                          {formatDate(viewingRental.endDate)}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-medium">
                          {viewingRental.installationTime}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-gray-600">Retirada:</span>
                        <span className="font-medium">
                          {viewingRental.removalTime}
                        </span>
                      </div>
                    </CardContent>
                  </Card>
                        ...
                        >
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-600">
                              {item.quantity}x • {item.days} dia(s) • R${' '}
                              {item.dailyRate.toFixed(2).replace('.', ',')}/dia
                            </p>
                          </div>
