...
import {
  ...
  pauseRecurrence,
  resumeRecurrence,
} from '../../../lib/database/recurring-rentals';
...
export default function RecurringRentalsPage() {
  ...
  const filteredRentals = useMemo(() => {
    ...
    if (statusFilter !== 'Todos') {
      filtered = filtered.filter(
        (rental: Rental) => rental.recurrenceStatus === statusFilter
      );
    }
    ...
    if (typeFilter !== 'Todos') {
      filtered = filtered.filter(
        (rental: Rental) => rental.recurrenceType === typeFilter
      );
    }
  ...
  const loadRecurringRentals = async () => {
    try {
      ...
      if (
        ...
        error.message.includes('COLUMNS_NOT_FOUND')
      ) {
        toast({
          title: 'Configuração necessária',
          description:
            'As colunas de recorrência não foram encontradas. Execute o script SQL de configuração primeiro.',
          variant: 'destructive',
        });
        setRentals([]);
      } else if (
        ...
        'message' in error &&
        typeof error.message === 'string' &&
        error.message.includes('recurring_rental_occurrences')
      ) {
        console.warn(
          'Tabela de ocorrências recorrentes não encontrada. Execute o script SQL primeiro.'
        );
        setRentals([]);
      } else {
        ...
        toast({
          ...
          description:
            'Não foi possível carregar as locações recorrentes. Tente novamente.',
          variant: 'destructive',
        });
      }
    } finally {
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-green-600">
                  {
                    filteredRentals.filter(r => r.recurrenceStatus === 'active')
                      .length
                  }
                </div>
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-yellow-600">
                  {
                    filteredRentals.filter(r => r.recurrenceStatus === 'paused')
                      .length
                  }
                </div>
              </CardContent>
                ...
                <div className="text-2xl font-bold text-red-600">
                  {
                    filteredRentals.filter(
                      r => r.recurrenceStatus === 'cancelled'
                    ).length
                  }
                </div>
              </CardContent>
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="font-medium">
                                    {getRecurrenceTypeLabel(
                                      rental.recurrenceType || 'none'
                                    )}
                                  </span>
                                </div>
                                <div className="text-sm text-text-secondary">
                                  A cada {rental.recurrenceInterval}{' '}
                                  {rental.recurrenceType === 'monthly'
                                    ? 'mês(es)'
                                    : rental.recurrenceType === 'weekly'
                                      ? 'semana(s)'
                                      : rental.recurrenceType === 'yearly'
                                        ? 'ano(s)'
                                        : 'dia(s)'}
                                </div>
                              </div>
        ...
        <AlertDialog open={actionDialogOpen} onOpenChange={setActionDialogOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                {actionType === 'pause' &&
                  'Tem certeza que deseja pausar esta recorrência? As próximas ocorrências não serão geradas automaticamente.'}
                {actionType === 'resume' &&
                  'Tem certeza que deseja retomar esta recorrência? As ocorrências voltarão a ser geradas automaticamente.'}
                {actionType === 'cancel' &&
                  'Tem certeza que deseja cancelar esta recorrência? Esta ação não pode ser desfeita e todas as ocorrências futuras serão removidas.'}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
