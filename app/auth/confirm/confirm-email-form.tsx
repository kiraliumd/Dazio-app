...
export function ConfirmEmailForm() {
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('🔍 Confirm Page: Parâmetros recebidos:', {
        token,
        type,
        error,
        errorCode,
    ...
    if (error || errorCode) {
      if (process.env.NODE_ENV === 'development') {
        console.log(
          '⚠️ Confirm Page: Erro detectado na URL:',
          error,
          errorCode
        );
      }
  ...
  const handleErrorCase = async () => {
    try {
      if (process.env.NODE_ENV === 'development') {
        console.log(
          '🔍 Confirm Page: Verificando se usuário já está autenticado...'
        );
      }
      ...
      if (user && user.email_confirmed_at) {
        if (process.env.NODE_ENV === 'development') {
          console.log(
            '✅ Confirm Page: Usuário já confirmado, redirecionando...'
          );
        }
        setStatus('success');
      ...
      setStatus('error');
      setMessage(
        'Link de confirmação expirado ou inválido. Por favor, faça login ou solicite um novo link.'
      );
      setLoading(false);
    } catch (error) {
      if (process.env.NODE_ENV === 'development') {
  ...
  const handleConfirmation = async (token: string) => {
    try {
      ...
      if (currentUser && !currentUser.email_confirmed_at) {
        if (process.env.NODE_ENV === 'development') {
          console.log(
            '🔍 Confirm Page: Usuário autenticado mas não confirmado, verificando token...'
          );
        }
        ...
        if (error) {
          ...
          const {
            data: { user: userAfterError },
          } = await supabase.auth.getUser();
          if (userAfterError && userAfterError.email_confirmed_at) {
            if (process.env.NODE_ENV === 'development') {
              console.log(
                '✅ Confirm Page: Usuário confirmado apesar do erro de token'
              );
            }
            setStatus('success');
          ...
          setStatus('error');
          setMessage(
            `Erro na confirmação: ${error.message}. Tente fazer login ou solicite um novo link.`
          );
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log(
            '🔍 Confirm Page: Usuário não autenticado, verificando token...'
          );
        }
  ...
  const createCompanyProfile = async () => {
    try {
      ...
      if (checkError && checkError.code !== 'PGRST116') {
        if (process.env.NODE_ENV === 'development') {
          console.error(
            '❌ Confirm Page: Erro ao verificar perfil existente:',
            checkError
          );
        }
        return;
      ...
      if (existingProfile) {
        if (process.env.NODE_ENV === 'development') {
          console.log(
            '✅ Confirm Page: Perfil da empresa já existe:',
            existingProfile.id
          );
        }
        return;
      ...
      const pendingProfileData = localStorage.getItem(
        `pendingProfileData_${user.id}`
      );
      if (!pendingProfileData) {
        if (process.env.NODE_ENV === 'development') {
          console.log(
            '⚠️ Confirm Page: Dados do perfil não encontrados (pode ser um usuário existente)'
          );
        }
        return;
      }
...
Data: {contract_date}`,
        ...
        if (settingsError) {
          if (process.env.NODE_ENV === 'development') {
            console.error(
              '❌ Confirm Page: Erro ao criar configurações:',
              settingsError
            );
          }
          return;
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(
          '✅ Confirm Page: Perfil e configurações criados com sucesso'
        );
      }
    } catch (error) {
      if (process.env.NODE_ENV === 'development') {
        console.error(
          '❌ Confirm Page: Erro ao criar perfil da empresa:',
          error
        );
      }
    }
