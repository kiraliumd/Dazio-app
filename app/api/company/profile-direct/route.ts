...
export async function GET() {
  try {
    ...
    console.log(
      '🔍 Company Profile Direct API: Authorization header:',
    ...
    if (!authorization) {
      console.log(
        '🔍 Company Profile Direct API: Tentando autenticação via cookies'
      );
      ...
      if (authError || !user) {
        console.error(
          '❌ Company Profile Direct API: Falha na autenticação via cookies:',
          authError
        );
      ...
      console.log(
        '✅ Company Profile Direct API: Usuário autenticado via cookies:',
        user.email
      );
      ...
      if (error) {
        console.error(
          '❌ Company Profile Direct API: Erro ao buscar perfil:',
          error
        );
        return NextResponse.json(
      ...
      console.log(
        '✅ Company Profile Direct API: Perfil encontrado via cookies'
      );
    ...
    console.log(
      '🔍 Company Profile Direct API: Token de autorização detectado (não implementado)'
    );
    return NextResponse.json(
      {
