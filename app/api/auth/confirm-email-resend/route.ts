...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      '🔍 Confirm Email Resend API: Processando confirmação com token:',
      token
    );
    ...
    if (tokenError || !tokenData) {
      console.error(
        '❌ Confirm Email Resend API: Token inválido ou não encontrado'
      );
      return NextResponse.json(
        {
          success: false,
    ...
    if (confirmError) {
      console.error(
        '❌ Confirm Email Resend API: Erro ao confirmar email:',
        confirmError
      );
      return NextResponse.json(
        {
  ...
  try {
    ...
    try {
      ...
      if (emailResult.success) {
        ...
        console.error(
          '❌ Resend Email API: Erro ao enviar email:',
          emailResult.error
        );
        return NextResponse.json(
          {
