  ...
  try {
    ...
    try {
      ...
      if (!audienceResult.success) {
        console.warn(
          '⚠️ Signup API: Erro ao adicionar à audiência:',
          audienceResult.error
        );
        // Não falha o cadastro se não conseguir adicionar à audiência
      } else {
        console.log(
          '✅ Signup API: Contato adicionado à audiência:',
          audienceResult.contactId
        );
      }
    } catch (audienceError) {
      console.warn(
        '⚠️ Signup API: Erro ao conectar com API de contatos:',
        audienceError
      );
    ...
    try {
      ...
      if (emailResult.success) {
        ...
        console.error(
          '❌ Signup API: Erro ao enviar email:',
          emailResult.error
        );
      }
    } catch (emailError) {
