...
export function DataCacheProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    dataService.setCacheContext({
      notifyDataChange: (dataType: keyof DataCache, operation: 'create' | 'update' | 'delete') => {
        if (process.env.NODE_ENV === "development") { console.log(`🔄 DataCacheContext: Recebendo notificação do DataService para ${dataType} (${operation})`)
  ...
  useEffect(() => {
    try {
      ...
      if (process.env.NODE_ENV === "development") { console.warn('Erro ao carregar cache do localStorage:', error)
    }
  }, [])
  ...
  useEffect(() => {
    try {
      ...
      if (process.env.NODE_ENV === "development") { console.warn('Erro ao salvar cache no localStorage:', error)
    }
  }, [cache])
  ...
  const invalidateCache = (key?: keyof DataCache) => {
    if (key) {
      setCache(prev => ({ ...prev, [key]: null }))
      if (process.env.NODE_ENV === "development") { console.log(`🗑️ DataCacheContext: Cache de ${key} invalidado`)
    } else {
      setCache({
        clients: null,
        equipments: null,
      ...
      if (process.env.NODE_ENV === "development") { console.log('🗑️ DataCacheContext: Todo o cache invalidado')
    }
  }
  ...
  const clearCache = () => {
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") { console.warn('Erro ao limpar cache do localStorage:', error)
    }
  }

  // Sistema de notificações para mudanças de dados
  const notifyDataChange = useCallback((dataType: keyof DataCache, operation: 'create' | 'update' | 'delete') => {
    if (process.env.NODE_ENV === "development") { console.log(`🔄 DataCacheContext: Notificando mudança em ${dataType} (${operation})`)
    ...
    changeSubscribers.forEach(callback => {
      try {
        ...
        if (process.env.NODE_ENV === "development") { console.warn('Erro ao executar callback de mudança:', error)
      }
    })
  }, [changeSubscribers])
