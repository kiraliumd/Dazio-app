...
export function TestSubscription() {
  ...
  const handleCreateSubscription = async (planType: 'monthly' | 'annual') => {
    if (process.env.NODE_ENV === "development") { console.log('🔄 TestSubscription: Iniciando criação de assinatura para:', planType); }
    setLoading(true);
    setDebugInfo(`Iniciando teste para ${planType}...`);
    
    try {
      if (process.env.NODE_ENV === "development") { console.log('📞 TestSubscription: Chamando API...'); }
      ...
      if (process.env.NODE_ENV === "development") { console.log('📋 TestSubscription: Resultado recebido:', result); }
      setDebugInfo(`Resultado: ${JSON.stringify(result, null, 2)}`);
      
      if (result.success && result.checkoutUrl) {
        if (process.env.NODE_ENV === "development") { console.log('✅ TestSubscription: Redirecionando para checkout...'); }
        setDebugInfo('Redirecionando para checkout do Stripe...');
        
        // Redirecionar para o checkout do Stripe
        window.location.href = result.checkoutUrl;
      } else {
        if (process.env.NODE_ENV === "development") { console.error('❌ TestSubscription: Erro na criação da assinatura:', result.error); }
        setDebugInfo(`Erro: ${result.error || 'Erro desconhecido'}`);
        toast.error(result.error || 'Erro ao criar assinatura');
      }
    } catch (error) {
      if (process.env.NODE_ENV === "development") { console.error('❌ TestSubscription: Erro inesperado:', error); }
