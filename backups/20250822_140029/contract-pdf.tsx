...
export function ContractPDF({ data }: ContractPDFProps) {
  ...
  if (!data) {
    if (process.env.NODE_ENV === "development") { console.error('ContractPDF: data is null or undefined'); }
    return null;
  }

  if (!data.client || !data.company || !data.contract) {
    if (process.env.NODE_ENV === "development") { console.error('ContractPDF: missing required data sections', { data }); }
    return null;
  }

  if (!data.contract.items || data.contract.items.length === 0) {
    if (process.env.NODE_ENV === "development") { console.error('ContractPDF: no items in contract'); }
    return null;
  }

  // Log para debug
  if (process.env.NODE_ENV === "development") { console.log('ContractPDF rendering with data:', data); }
