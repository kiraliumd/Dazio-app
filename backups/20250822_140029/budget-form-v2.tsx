...
export function BudgetFormV2({ open, onOpenChange, budget, onSave }: BudgetFormProps) {
  ...
  useEffect(() => {
    if (clientsError) {
      if (process.env.NODE_ENV === "development") { console.error('Erro ao carregar clientes:', clientsError)
    }
    if (equipmentsError) {
      if (process.env.NODE_ENV === "development") { console.error('Erro ao carregar equipamentos:', equipmentsError)
    }
  ...
  useEffect(() => {
    if (open) {
      ...
      if (process.env.NODE_ENV === "development") { console.log('📦 BudgetForm: Dados sendo carregados pelos hooks otimizados')
    }
  }, [open])
    ...
    if (formData.isRecurring && formData.startDate && formData.recurrenceType && formData.recurrenceInterval > 0) {
      try {
        ...
        if (isNaN(startDate.getTime())) {
          if (process.env.NODE_ENV === "development") { console.error("Data de início inválida:", formData.startDate)
          return
        }
        ...
        if (process.env.NODE_ENV === "development") { console.error("Erro ao calcular datas de recorrência:", error)
      }
    }
  ...
  const handleSaveBudget = async () => {
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") { console.error("Erro ao salvar orçamento:", error)
