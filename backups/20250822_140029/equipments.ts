...
export async function getEquipments() {
  ...
  if (!companyId) {
    if (process.env.NODE_ENV === "development") { console.error('❌ getEquipments: Company ID não encontrado')
...
export async function getEquipmentById(id: string) {
  ...
  if (!companyId) {
    if (process.env.NODE_ENV === "development") { console.error('❌ getEquipmentById: Company ID não encontrado')
    throw new Error('Usuário não autenticado ou empresa não encontrada')
  }
  ...
  if (error) {
    if (process.env.NODE_ENV === "development") { console.error("Erro ao buscar equipamento:", error)
    throw error
  }
...
export async function createEquipment(equipment: Omit<Equipment, "id" | "created_at" | "updated_at">) {
  ...
  if (!companyId) {
    if (process.env.NODE_ENV === "development") { console.error('❌ createEquipment: Company ID não encontrado')
    throw new Error('Usuário não autenticado ou empresa não encontrada')
  }
  ...
  if (error) {
    if (process.env.NODE_ENV === "development") { console.error("Erro ao criar equipamento:", error)
    throw error
  }
...
export async function updateEquipment(
  ...
  if (!companyId) {
    if (process.env.NODE_ENV === "development") { console.error('❌ updateEquipment: Company ID não encontrado')
    throw new Error('Usuário não autenticado ou empresa não encontrada')
  }
  ...
  if (error) {
    if (process.env.NODE_ENV === "development") { console.error("Erro ao atualizar equipamento:", error)
    throw error
  }
...
export async function deleteEquipment(id: string) {
  ...
  if (!companyId) {
    if (process.env.NODE_ENV === "development") { console.error('❌ deleteEquipment: Company ID não encontrado')
    throw new Error('Usuário não autenticado ou empresa não encontrada')
  }
  ...
  if (error) {
    if (process.env.NODE_ENV === "development") { console.error("Erro ao deletar equipamento:", error)
    throw error
  }
...
export async function searchEquipments(searchTerm?: string, categoryFilter?: string, statusFilter?: string) {
  ...
  if (!companyId) {
    if (process.env.NODE_ENV === "development") { console.error('❌ searchEquipments: Company ID não encontrado')
    throw new Error('Usuário não autenticado ou empresa não encontrada')
  }
...
export async function checkEquipmentAvailability(
  ...
  if (rentalsError) {
    if (process.env.NODE_ENV === "development") { console.error("Erro ao buscar locações ativas:", rentalsError)
  ...
  if (rentedError) {
    if (process.env.NODE_ENV === "development") { console.error("Erro ao verificar equipamentos alugados:", rentedError)
    return { available: false, message: "Erro ao verificar disponibilidade" }
  }
