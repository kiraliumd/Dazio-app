...
export function SimpleTest() {
  ...
  const handleTest = async (planType: 'monthly' | 'annual') => {
    if (process.env.NODE_ENV === "development") { console.log('🔄 Iniciando teste para:', planType); }
    setLoading(true);
    setMessage(`Testando ${planType}...`);
    
    try {
      // Simular uma chamada de API
      if (process.env.NODE_ENV === "development") { console.log('📞 Fazendo chamada para createSubscription...'); }
      ...
      if (process.env.NODE_ENV === "development") { console.log('✅ Teste concluído com sucesso'); }
      
    } catch (error) {
      if (process.env.NODE_ENV === "development") { console.error('❌ Erro no teste:', error); }
