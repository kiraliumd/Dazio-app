...
export async function getCompanySettings(): Promise<CompanySettings> {
  ...
  if (error && error.code !== "PGRST116") { // PGRST116: a consulta não retornou linhas
    if (process.env.NODE_ENV === "development") { console.error("Erro ao buscar configurações da empresa:", error); }
    throw error;
  }
...
export async function updateCompanySettings(
  ...
  if (error) {
    if (process.env.NODE_ENV === "development") { console.error("Erro ao atualizar configurações da empresa:", error); }
    throw error;
  }
