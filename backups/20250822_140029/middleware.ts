...
export async function middleware(req: NextRequest) {
  ...
  try {
    ...
    if (isTrialExpired && isTrialStatus) {
      ...
      if (process.env.NODE_ENV === "development") { console.log('Middleware: Trial expirado, redirecionando para assinatura-gestao'); }
    ...
    if (['cancelled', 'expired'].includes(profile.status)) {
      if (!SUBSCRIPTION_ROUTES.includes(pathname)) {
        if (process.env.NODE_ENV === "development") { console.log('Middleware: Assinatura cancelada/expirada, redirecionando para assinatura-gestao'); }
        return NextResponse.redirect(new URL('/assinatura-gestao', req.url));
      }
    }
    ...
    if (process.env.NODE_ENV === "development") { console.error('Middleware: Erro ao verificar trial:', error); }
