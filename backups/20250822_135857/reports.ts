...
export async function getRentalsForReports(filters: ReportFilters): Promise<RentalReport[]> {
  try {
    ...
    if (!companyId) {
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error('❌ getRentalsForReports: Company ID não encontrado')
    ...
    if (error) {
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao buscar locações para relatórios:", error)
      throw error
    }
...
export async function getBudgetsForReports(filters: ReportFilters): Promise<BudgetReport[]> {
  try {
    ...
    if (!companyId) {
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error('❌ getBudgetsForReports: Company ID não encontrado')
      throw new Error('Usuário não autenticado ou empresa não encontrada')
    }
    ...
    if (error) {
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao buscar orçamentos para relatórios:", error)
      throw error
    }
...
export async function getAllRentalsForReports(): Promise<RentalReport[]> {
  try {
    ...
    if (error) {
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao buscar todas as locações:", error)
      throw error
    }
...
export async function getAllBudgetsForReports(): Promise<BudgetReport[]> {
  try {
    ...
    if (error) {
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao buscar todos os orçamentos:", error)
      throw error
    }
