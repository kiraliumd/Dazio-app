...
export function TestCacheSystem() {
  ...
  const handleCreateTestBudget = async () => {
    try {
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.log('🧪 Teste: Criando orçamento de teste...')
      const result = await createBudget(testBudget, testItems)
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.log('✅ Teste: Orçamento criado:', result)
      
      // O cache deve ser invalidado automaticamente
      // e a lista deve ser atualizada em tempo real
    } catch (error) {
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error('❌ Teste: Erro ao criar orçamento:', error)
    }
  }

  const handleUpdateTestBudget = async (id: string) => {
    try {
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.log('🧪 Teste: Atualizando orçamento...')
      const updateData = { ...testBudget, totalValue: 950 }
      const result = await updateBudget(id, updateData, testItems)
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.log('✅ Teste: Orçamento atualizado:', result)
      
      // O cache deve ser invalidado automaticamente
      // e as alterações devem aparecer imediatamente
    } catch (error) {
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error('❌ Teste: Erro ao atualizar orçamento:', error)
    }
  }

  const handleDeleteTestBudget = async (id: string) => {
    try {
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.log('🧪 Teste: Excluindo orçamento...')
      const result = await deleteBudget(id)
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.log('✅ Teste: Orçamento excluído:', result)
      
      // O cache deve ser invalidado automaticamente
      // e o orçamento deve ser removido da lista
    } catch (error) {
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error('❌ Teste: Erro ao excluir orçamento:', error)
    }
  }

  const handleForceRefresh = () => {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.log('🧪 Teste: Forçando refresh manual...')
    forceRefresh()
  }
