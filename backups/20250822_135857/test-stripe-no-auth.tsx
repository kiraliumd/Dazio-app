...
export function TestStripeNoAuth() {
  ...
  const handleTest = async (planType: 'monthly' | 'annual') => {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.log('🔄 TestStripeNoAuth: Iniciando teste para:', planType); } }
    setLoading(true);
    setDebugInfo(`Iniciando teste para ${planType}...`);
    
    try {
      // Testar diretamente a API do Stripe sem autenticação
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.log('📞 TestStripeNoAuth: Testando Stripe diretamente...'); } }
      ...
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.log('📋 TestStripeNoAuth: Resposta do servidor:', response.status); } }
      setDebugInfo(`Resposta do servidor: ${response.status}`);
      
      if (response.ok) {
        const data = await response.json();
        if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.log('✅ TestStripeNoAuth: Dados recebidos:', data); } }
        ...
        if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error('❌ TestStripeNoAuth: Erro na resposta:', errorData); } }
        setDebugInfo(`Erro na resposta: ${JSON.stringify(errorData, null, 2)}`);
      }
      
    } catch (error) {
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error('❌ TestStripeNoAuth: Erro na chamada:', error); } }
