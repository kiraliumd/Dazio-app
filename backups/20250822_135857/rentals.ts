...
function calculateNextOccurrenceDate(rental: any): string | null {
  ...
  try {
    ...
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao calcular próxima ocorrência:", error)
    return null
  }
}
...
export async function getRentals(limit?: number) {
  ...
  if (!companyId) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error('❌ getRentals: Company ID não encontrado')
  ...
  if (error) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao buscar locações:", error)
    throw error
  }
...
export async function getRentalById(id: string) {
  ...
  if (!companyId) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error('❌ getRentalById: Company ID não encontrado')
    throw new Error('Usuário não autenticado ou empresa não encontrada')
  }
  ...
  if (error) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao buscar locação:", error)
    throw error
  }
...
export async function createRental(
  ...
  if (!companyId) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error('❌ createRental: Company ID não encontrado')
    throw new Error('Usuário não autenticado ou empresa não encontrada')
  }
  ...
  if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.log("Dados a serem inseridos:", rentalData)
  if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.log("Período da locação:", rental.start_date, "até", rental.end_date)
  if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.log("Datas de logística:", logisticsData.installation, "e", logisticsData.removal)
  if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.log("Dados de recorrência:", {
  ...
  // 1. Criar a locação
  if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.log("Tentando inserir locação com dados:", JSON.stringify(rentalData, null, 2))
  
  const { data: createdRental, error: rentalError } = await supabase.from("rentals").insert([rentalData]).select().single()

  if (rentalError) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro detalhado ao criar locação:", {
  ...
  if (items.length > 0) {
    ...
    if (itemsError) {
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao criar itens da locação:", itemsError)
  ...
  try {
    ...
    if (logisticsError) {
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao criar eventos de logística:", logisticsError)
      // Não vamos falhar a criação da locação por causa dos eventos
    }
  } catch (error) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao criar eventos de logística:", error)
    // Não vamos falhar a criação da locação por causa dos eventos
  }
...
export async function updateRental(
  ...
  if (!companyId) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error('❌ updateRental: Company ID não encontrado')
    throw new Error('Usuário não autenticado ou empresa não encontrada')
  }
  ...
  if (rentalError) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao atualizar locação:", rentalError)
    throw rentalError
  }
  ...
  if (items) {
    ...
    if (items.length > 0) {
      ...
      if (itemsError) {
        if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao atualizar itens da locação:", itemsError)
        throw itemsError
      }
    }
  }
...
export async function deleteRental(id: string) {
  ...
  if (!companyId) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error('❌ deleteRental: Company ID não encontrado')
    throw new Error('Usuário não autenticado ou empresa não encontrada')
  }
  ...
  if (error) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao deletar locação:", error)
    throw error
  }
...
export async function searchRentals(searchTerm?: string, statusFilter?: string) {
  ...
  if (!companyId) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error('❌ searchRentals: Company ID não encontrado')
    throw new Error('Usuário não autenticado ou empresa não encontrada')
  }
...
export async function getLogisticsEvents() {
  ...
  if (!companyId) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error('❌ getLogisticsEvents: Company ID não encontrado')
    throw new Error('Usuário não autenticado ou empresa não encontrada')
  }
  ...
  if (error) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao buscar eventos de logística:", error)
    throw error
  }
