  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === '42703' && error.message.includes('is_recurring')) {
        throw new Error('COLUMNS_NOT_FOUND: As colunas de recorrência não foram encontradas. Execute o script SQL primeiro.')
      }
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao buscar locações recorrentes:", error)
      throw error
    }
        ...
        data.map(async (rental) => {
          try {
            ...
            if (occurrenceError.code === '42P01') {
              if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.warn("Tabela recurring_rental_occurrences não encontrada")
              return {
                ...rental,
...
export async function getRecurringOccurrences(parentRentalId: string) {
  ...
  if (error) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao buscar ocorrências:", error)
    throw error
  }
...
export async function createRecurringRental(
  ...
  if (rentalError) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao criar locação:", rentalError)
    throw rentalError
  }
...
export async function generateFutureOccurrences(rentalId: string, maxOccurrences: number = 12) {
  ...
  if (error) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao gerar ocorrências futuras:", error)
    throw error
  }
}
...
export async function pauseRecurrence(rentalId: string) {
  ...
  if (error) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao pausar recorrência:", error)
    throw error
  }
}
...
export async function resumeRecurrence(rentalId: string) {
  ...
  if (error) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao retomar recorrência:", error)
    throw error
  }
}
...
export async function cancelRecurrence(rentalId: string) {
  ...
  if (error) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao cancelar recorrência:", error)
    throw error
  }
}
...
export async function updateOccurrence(
  ...
  if (error) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao atualizar ocorrência:", error)
    throw error
  }
}
...
export async function getUpcomingOccurrences(limit: number = 10) {
  ...
  if (error) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao buscar próximas ocorrências:", error)
    throw error
  }
...
export async function getRecurrenceStats() {
  ...
  if (error) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error("Erro ao buscar estatísticas:", error)
    throw error
  }
  ...
  const stats = {
    ...
    cancelled: data?.filter(r => r.recurrence_status === "cancelled").length || 0,
