...
export async function sendConfirmationEmail(email: string, token: string) {
  ...
  try {
    ...
    if (error) {
      if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error('Erro ao enviar email:', error); } }
      throw error;
    }

    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.log('Email enviado com sucesso:', data); } }
    return data;
  } catch (error) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error('Erro ao enviar email de confirmação:', error); } }
    throw error;
  }
}
...
export async function sendPasswordResetEmail(email: string, token: string) {
  ...
  try {
    ...
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.log('Email de redefinição enviado com sucesso:', data); } }
    return data;
  } catch (error) {
    if (process.env.NODE_ENV === "development") { if (process.env.NODE_ENV === "development") { console.error('Erro ao enviar email de redefinição:', error); } }
    throw error;
  }
} 