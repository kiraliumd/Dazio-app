...
export async function getCurrentUserCompanyIdServer(): Promise<string | null> {
  try {
    ...
    if (!user) {
      console.error(
        '❌ getCurrentUserCompanyIdServer: Usuário não autenticado'
      );
      return null;
    }
    ...
    if (error) {
      console.error(
        '❌ getCurrentUserCompanyIdServer: Erro ao buscar perfil da empresa:',
        error
      );
      return null;
    }
...
export async function getCompanyIdByUserId(
  ...
  try {
    ...
    if (error) {
      console.error(
        '❌ getCompanyIdByUserId: Erro ao buscar perfil da empresa:',
        error
      );
      return null;
    }
