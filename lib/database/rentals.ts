...
function calculateNextOccurrenceDate(rental: any): string | null {
  if (
    !rental.is_recurring ||
    !rental.recurrence_type ||
    rental.recurrence_type === 'none'
  ) {
    return null;
  }
...
export async function createRental(
  ...
  rentalData.next_occurrence_date =
    rental.next_occurrence_date || calculateNextOccurrenceDate(rental);
  ...
  console.log(
    'Datas de logística:',
    logisticsData.installation,
    'e',
    logisticsData.removal
  );
  ...
  // 1. Criar a locação
  console.log(
    'Tentando inserir locação com dados:',
  ...
  if (rentalError) {
    ...
    throw new Error(
      `Erro ao criar locação: ${rentalError.message} - ${rentalError.details || ''}`
    );
  }
...
export async function updateRental(
  ...
  if (rental.client_name !== undefined)
    updateData.client_name = rental.client_name;
  ...
  if (rental.installation_location !== undefined)
    updateData.installation_location = rental.installation_location;
  if (rental.total_value !== undefined)
    updateData.total_value = rental.total_value;
  if (rental.discount !== undefined) updateData.discount = rental.discount;
  if (rental.final_value !== undefined)
    updateData.final_value = rental.final_value;
...
export async function searchRentals(
  ...
  if (searchTerm) {
    query = query.or(
      `client_name.ilike.%${searchTerm}%,status.ilike.%${searchTerm}%`
    );
  }
