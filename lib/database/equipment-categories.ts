...
export async function createEquipmentCategory(
  category: CreateEquipmentCategory
) {
  const companyId = await getCurrentUserCompanyId();
...
export async function updateEquipmentCategory(
  id: string,
  category: UpdateEquipmentCategory
) {
  const companyId = await getCurrentUserCompanyId();
...
export async function deleteEquipmentCategory(id: string) {
  ...
  if (equipments && equipments.length > 0) {
    throw new Error(
      `Não é possível deletar esta categoria. Existem ${equipments.length} equipamento(s) associado(s).`
    );
  }
