...
export async function testAuthenticationAndAccess() {
  ...
  try {
    ...
    if (profileError) {
      console.error(
        '❌ TESTE: Erro ao acessar company_profiles:',
        profileError
      );
      return {
        success: false,
        error: 'Erro ao acessar company_profiles',
    ...
    if (budgetsError) {
      ...
      return {
        success: false,
        error: 'Erro ao acessar budgets',
        details: budgetsError,
      };
    }

    console.log(
      '✅ TESTE: Budgets acessados com sucesso:',
      budgets?.length || 0,
      'registros'
    );
    ...
    if (clientsError) {
      ...
      return {
        success: false,
        error: 'Erro ao acessar clients',
        details: clientsError,
      };
    }

    console.log(
      '✅ TESTE: Clients acessados com sucesso:',
      clients?.length || 0,
      'registros'
    );
    ...
    if (equipmentsError) {
      ...
      return {
        success: false,
        error: 'Erro ao acessar equipments',
        details: equipmentsError,
      };
    }

    console.log(
      '✅ TESTE: Equipments acessados com sucesso:',
      equipments?.length || 0,
      'registros'
    );
...
export async function testDatabaseFunction() {
  ...
  try {
    ...
    if (error) {
      ...
      return {
        success: false,
        error: 'Erro na função get_user_company_id',
        details: error,
      };
    }
