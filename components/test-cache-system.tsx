...
export function TestCacheSystem() {
  ...
  const handleCreateTestBudget = async () => {
    try {
      console.log('🧪 Teste: Criando orçamento de teste...');
      const result = await createBudget(testBudget, testItems);
      console.log('✅ Teste: Orçamento criado:', result);

      // O cache deve ser invalidado automaticamente
      // e a lista deve ser atualizada em tempo real
    } catch (error) {
      console.error('❌ Teste: Erro ao criar orçamento:', error);
    }
  };

  const handleUpdateTestBudget = async (id: string) => {
    try {
      console.log('🧪 Teste: Atualizando orçamento...');
      const updateData = { ...testBudget, totalValue: 950 };
      const result = await updateBudget(id, updateData, testItems);
      console.log('✅ Teste: Orçamento atualizado:', result);

      // O cache deve ser invalidado automaticamente
      // e as alterações devem aparecer imediatamente
    } catch (error) {
      console.error('❌ Teste: Erro ao atualizar orçamento:', error);
    }
  };

  const handleDeleteTestBudget = async (id: string) => {
    try {
      console.log('🧪 Teste: Excluindo orçamento...');
      const result = await deleteBudget(id);
      console.log('✅ Teste: Orçamento excluído:', result);

      // O cache deve ser invalidado automaticamente
      // e o orçamento deve ser removido da lista
    } catch (error) {
      console.error('❌ Teste: Erro ao excluir orçamento:', error);
    }
  };

  const handleForceRefresh = () => {
    console.log('🧪 Teste: Forçando refresh manual...');
    forceRefresh();
  };
    ...
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          Teste o sistema de cache em tempo real. Crie, edite e exclua
          orçamentos para ver como o cache é invalidado automaticamente e a
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="p-3 bg-purple-50 rounded-lg">
            ...
            <div className="text-xs text-purple-600">Sistema ativo</div>
          </div>
        </div>
          ...
          {budgets && budgets.length > 0 ? (
            ...
            <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg text-center">
              <div className="text-sm text-gray-600">
                Nenhum orçamento encontrado. Crie um orçamento de teste para
                começar!
              </div>
            </div>
          )}
        ...
        <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          <ol className="text-sm text-blue-800 space-y-1 list-decimal list-inside">
            <li>
              Clique em &quot;Criar Orçamento Teste&quot; para adicionar um novo
              orçamento
            </li>
            <li>
              Observe como ele aparece imediatamente na lista (sem refresh)
            </li>
            <li>
              Clique em &quot;Editar&quot; para modificar um orçamento existente
            </li>
          ...
          <div className="mt-3 p-2 bg-blue-100 rounded text-xs text-blue-800">
            💡 <strong>Dica:</strong> Abra o console do navegador para ver os
            logs de invalidação de cache em tempo real!
          </div>
        </div>
      </CardContent>
    </Card>
